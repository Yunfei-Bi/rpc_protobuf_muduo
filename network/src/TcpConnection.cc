...
// defaultMessageCallback：默认的消息回调函数，用于清空输入缓冲区
...
// getTcpInfo：调用 Socket 对象的 getTcpInfo 方法，获取 TCP 连接的详细信息。
...
// FIXME efficiency!!!
// send：如果连接状态为 kConnected，根据当前线程是否为事件循环线程，
// 选择直接调用 sendInLoop 
// 或通过 EventLoop 的 runInLoop 方法在事件循环线程中调用 sendInLoop
...
// sendInLoop：在事件循环线程中发送数据。如果输出缓冲区为空，尝试直接写入数据；
// 如果写入成功且数据全部发送完毕，调用写完成回调函数；
// 如果写入失败且不是 EWOULDBLOCK 错误，记录错误信息；
// 如果还有剩余数据，将其添加到输出缓冲区，并启用写事件。
...
// forceClose：如果连接状态为 kConnected 或 kDisconnecting，
// 将状态设置为 kDisconnecting，并在事件循环线程中调用 forceCloseInLoop
...
// forceCloseWithDelay：如果连接状态为 kConnected 或 kDisconnecting，
