...
Acceptor::Acceptor(EventLoop *loop, const InetAddress &listenAddr,
    ...
    : loop_(loop), // 关联的事件循环
      ...
      // 为监听套接字创建一个 Channel 对象，用于事件处理
      acceptChannel_(loop, acceptSocket_.fd()),
      // 初始化为 false，表示还未开始监听
      listening_(false),
      // 打开 /dev/null 文件，用于处理文件描述符耗尽的情况
...
void Acceptor::handleRead() {
  ...
  // 调用 acceptSocket_.accept(&peerAddr) 接受新连接，
  // 获取新连接的文件描述符 connfd 和客户端地址 peerAddr
  int connfd = acceptSocket_.accept(&peerAddr);
  if (connfd >= 0) { // 如果 connfd >= 0，表示成功接受新连接
    // string hostport = peerAddr.toIpPort();
    // LOG_TRACE << "Accepts of " << hostport;
    // 如果 newConnectionCallback_ 不为空，调用该回调函数处理新连接
